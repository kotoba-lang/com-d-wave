(ns d_wave.main
  "Kotodama WASM entrypoint for the D_wave clean-room actor (L4) — Clojure port.
  ...
  facts (`d_wave.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "d_wave")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"d_wave.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "d_wave-compat" :tier tier :entities entities} 200])
...
(defn start! [] :d_wave-compat/ready)
